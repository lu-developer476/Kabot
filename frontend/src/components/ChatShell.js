...
const THEMES = {
  dark: { label: { es: 'Oscuro', en: 'Dark' }, icon: '●' },
  blue: { label: { es: 'Azul', en: 'Blue' }, icon: '◆' },
  light: { label: { es: 'Claro', en: 'Light' }, icon: '○' },
  beige: { label: { es: 'Beige', en: 'Beige' }, icon: '◒' },
};
...
export default function ChatShell() {
  ...
  const [theme, setTheme] = useState('dark'); const [language, setLanguage] = useState('es'); const [timezone, setTimezone] = useState('America/Argentina/Buenos_Aires'); const [responseStyle, setResponseStyle] = useState('balanced'); const [clock, setClock] = useState(new Date()); const [copiedId, setCopiedId] = useState('');
  ...
  const worldTime = useMemo(() => new Intl.DateTimeFormat(language === 'es' ? 'es-AR' : 'en-US', { timeZone: timezone, hour: '2-digit', minute: '2-digit', weekday: 'short', day: '2-digit', month: 'short' }).format(clock), [clock, language, timezone]);

  useEffect(() => { const saved = JSON.parse(localStorage.getItem('kabot.preferences.v2') || '{}'); setTheme(saved.theme || 'dark'); setLanguage(saved.language || 'es'); setTimezone(saved.timezone || 'America/Argentina/Buenos_Aires'); setResponseStyle(saved.responseStyle || 'balanced'); }, []);
  useEffect(() => { document.documentElement.dataset.theme = theme; localStorage.setItem('kabot.preferences.v2', JSON.stringify({ theme, language, timezone, responseStyle })); }, [theme, language, timezone, responseStyle]);
  ...
  const createConversation = async () => { try { setLoading(true); setError(''); const created = await request('/api/chats', { method: 'POST', body: JSON.stringify({}) }, CHAT_INIT_TIMEOUT_MS, CHAT_INIT_TIMEOUT_MESSAGE); setChats((prev) => [created.chat, ...prev]); setChatId(created.chat.id); setMessages([]); setInput(''); setChatStatus('ready'); } catch (err) { setError(err.message || 'No se pudo crear el chat.'); } finally { setLoading(false); } };
  const selectChat = async (selectedChatId) => { if (selectedChatId === chatId || loading) return; try { setLoading(true); setError(''); setChatId(selectedChatId); await loadMessages(selectedChatId); setChatStatus('ready'); } catch (err) { setError(err.message || 'No se pudo cargar la conversación.'); } finally { setLoading(false); } };
